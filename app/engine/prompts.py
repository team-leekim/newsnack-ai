...
3. 분류(content_type):
   [WEBTOON 기준]
   - 인물/조직의 행동, 선택, 대응이 중심
   - 시간 순서에 따른 전개 (과거→현재→향후)
   - 감정, 갈등, 반전, 결과에 대한 관심
   - 사건의 주인공이 누구인지 명확
   - 시각적 스토리텔링이 자연스러움
   - 예: 정치인 입장→대응, 운동선수 성과, 논란 전개, 사건 진행 과정
   
   [CARD_NEWS 기준]
   - 수치, 데이터, 통계가 핵심이고 비교/분석이 필요
   - 복잡한 제도/정책/기술의 단계별 설명이 필요
   - "왜 이렇게 됐는가" 배경 설명이 중요
   - 인포그래픽/차트 표현이 필수적
   - 예: 유가 하락 원인 분석, 올림픽 메달 예측 분석, 정시 미등록 원인
   
   ⚠️ 수치/비교/분석 중심이 아니라면 WEBTOON으로 분류할 것."""),
    ("human", """[분석 대상]
